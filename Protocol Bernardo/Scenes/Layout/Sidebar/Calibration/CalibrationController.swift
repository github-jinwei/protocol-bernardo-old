...
extension CalibrationController {
    ...
    func storeDeltas() {
        ...
        deviceProfile?.orientationDelta = Double(deltas.orientation ?? 0.0)
        deviceProfile?.positionDelta.x = Double(deltas.xPosition)
        deviceProfile?.positionDelta.y = Double(deltas.yPosition)
        deviceProfile?.heightDelta = Double(deltas.height)
    }
}
...
extension CalibrationController: DataAcquisitionEngineObserver {
    ...
    ///
    /// - Parameter connectedDevices: Connected devices from the dae
    var physicalDeviceState: DeviceState {
        guard let deviceSerial = self.deviceSerial else { return DEVICE_UNKNOWN }

        guard let physicalDevice = App.dae.connectedDevices.with(serial: deviceSerial) else { return DEVICE_UNKNOWN }
...
extension CalibrationController {
    fileprivate func calculateDeltas() {
        ...
        let physicalDevice = App.dae.connectedDevices.with(serial: deviceSerial)!
